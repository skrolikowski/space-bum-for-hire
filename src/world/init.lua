...
function World:queryScreen(callback)
	if callback then
		self.world:queryBoundingBox(0, 0, Config.width, Config.height, callback)
	else
		return self.world:fetchInBounds(self:bounds())
	end
end
...
--
function World:fetchInBounds(bounds)
	local fixtures = {}
	local left,
	      top,
	      right,
	      bottom   = bounds:unpack()
